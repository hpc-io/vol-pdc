 ...
 */
#define HERROR_DUMP_STACK(err_stack)                                                                         \
    do {                                                                                                     \
        H5E_auto2_t func;                                                                                    \
...
#define FUNC_LEAVE_VOL_NAME(name)                                                                            \
    ...
    return __ret_value;
#define FUNC_LEAVE_VOL FUNC_LEAVE_VOL_NAME(H5VL_ERR_NAME)

/**
 * Use this macro to set return value.
 */
#define FUNC_RETURN_SET(ret) (__ret_value = ret)

/**
 * Use this macro to test for error.
 */
#define FUNC_ERRORED (__err_occurred == TRUE)
 ...
 */
#define HERROR(err_stack, err_cls, maj_id, min_id, ...)                                                      \
    do {                                                                                                     \
        H5E_auto2_t func;                                                                                    \
        /* Check whether automatic error reporting has been disabled */                                      \
        H5Eget_auto2(H5E_DEFAULT, &func, NULL);                                                              \
        if (func && err_stack > 0 && err_cls > 0) {                                                          \
            H5Epush2(err_stack, __FILE__, __func__, __LINE__, err_cls, maj_id, min_id, __VA_ARGS__);         \
        }                                                                                                    \
    } while (0)
 ...
 */
#define HCOMMON_ERROR_NAME(name, maj, min, ...)                                                              \
    do {                                                                                                     \
        HERROR(H5VL_ERR_STACK_NAME(name), H5VL_ERR_CLS_NAME(name), maj, min, __VA_ARGS__);                   \
        __err_occurred = TRUE;                                                                               \
    } while (0)
#define HCOMMON_ERROR(maj, min, ...) HCOMMON_ERROR_NAME(H5VL_ERR_NAME, maj, min, __VA_ARGS__)
