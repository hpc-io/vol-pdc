...
void
create_h5_datasets(int rank, int i)
{
    char dname[128];
...
main(int argc, char *argv[])
{
    ...
    t3 = MPI_Wtime();
    if (my_rank == 0)
        printf("Wrote %d files, took %.2f, actual write took %.2f\n", nstep, t3 - t0,
               t2 - t0 - sleeptime * (nstep - 1));
